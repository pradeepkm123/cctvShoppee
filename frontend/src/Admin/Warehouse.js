...
const WarehouseDetails = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (currentWarehouse) {
        ...
        await axios.put(`http://localhost:5000/api/warehouses/${currentWarehouse._id}`, formData);
      } else {
        // Add new warehouse
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/warehouses/${currentWarehouse._id}`);
      fetchWarehouses();
