...
//       const token = localStorage.getItem('token');
//       await axios.put(
//         `http://localhost:5000/api/cart/update/${updatedCart[index].product._id}`,
...
//     return cart.reduce((total, item) => total + ((item.product.newPrice ?? 0) * item.quantity), 0);
//   };
...
//     return total > 1000 ? total * 0.10 : 0;
//   };

//   const calculateDeliveryCharges = () => {
//     return calculateTotal() >= 2000 ? 0 : 100;
//   };
...
//     const camerasInCart = cart.filter(item => item.product.name.toLowerCase().includes('camera'));
//     return includeInstallation ? camerasInCart.length * 700 : 0;
//   };

//   // 👇 force GST to integer
//   const calculateGST = (total) => {
//     return Math.round(total * 0.18);
//   };

//   // 👇 force Total Amount to integer
//   const totalAmount = () => {
...
//                                 <span className="tp-cart-minus" onClick={() => handleDecrement(index)}>
...
//                                 <span className="tp-cart-plus" onClick={() => handleIncrement(index)}>
