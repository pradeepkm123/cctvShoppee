...
//   // normalize windows slashes and strip leading "uploads/"
...
//     for (let s = 1; s <= 5; s++) {
//       const c = counts[s];
//       if (c > highestCount || (c === highestCount && s > highestStar)) {
//         highestCount = c;
//         highestStar = s;
//       }
//     }
...
//     const v = Math.round(Math.max(0, Math.min(5, Number(value) || 0)));
//     return (
//       <>
...
//         // pick first media
//         const firstMedia =
//           (data.media && data.media.length && data.media[0]) ||
//           (data.images && data.images[0] && { type: 'image', file: data.images[0] }) ||
//           (data.videos && data.videos[0] && { type: 'video', file: data.videos[0] }) ||
//           null;

//         if (firstMedia) {
//           const url = toAbsUrl(firstMedia.file || firstMedia);
//           const typeGuess =
//             firstMedia.type || (data.images?.some((x) => toAbsUrl(x) === url) ? 'image' : 'video');
...
//       try {
//         const { data } = await axios.get(`${API_BASE}/reviews/${id}`);
...
//     try {
//       const resp = await axios.post(`${API_BASE}/reviews`, {
//         productId: id,
//         name: name.value,
...
//       return product.media.map((m) => ({ type: m.type, url: toAbsUrl(m.file) }));
//     }
//     const out = [];
//     (product.images || []).forEach((u) => out.push({ type: 'image', url: toAbsUrl(u) }));
//     (product.videos || []).forEach((u) => out.push({ type: 'video', url: toAbsUrl(u) }));
//     return out;
//   }, [product]);

//   const onThumbClick = (m) => setActiveMedia(m);
...
//   const moveDown = () =>
//     setThumbStart((s) => Math.min(Math.max(0, thumbs.length - VISIBLE_THUMBS), s + 1));
...
//                             >
//                               {m.type === 'image' ? (
//                                 <img src={m.url} alt={`thumb-${idx}`} />
//                               ) : (
//                                 <div className="tp-thumb-video">
//                                   <video src={m.url} muted />
//                                   <span className="tp-thumb-badge">▶</span>
//                                 </div>
//                               )}
//                             </button>
//                           );
...
//                             product.stock > 0 ? 'text-emerald-600 bg-emerald-100' : 'text-red-600 bg-red-100'
//                           }`}
//                         >
...
//                           {renderStars(ratingStats.highestStar)}
//                         </div>
...
//                           <button className="tp-product-details-add-to-cart-btn w-100" onClick={handleAddToCart} type="button">
//                             <i className="las la-shopping-cart" style={{ fontSize: '20px' }}></i> Add To Cart
//                           </button>
//                         </div>
//                       </div>

//                       {/* <div className="tp-product-details-action-item-wrapper d-flex align-items-center" style={{ gap: 12 }}>
//                         <button className="tp-product-details-buy-now-btn" type="button" onClick={handleBuyNow}>
//                           <i className="las la-bolt" style={{ fontSize: '20px' }}></i> Buy Now
//                         </button>
//                       </div> */}
//                     </div>

//                     <div className="tp-product-details-action-sm">
//                       <button type="button" className="tp-product-details-action-sm-btn" onClick={handleAddToWishlist}>
...
//                         <i className="las la-exchange-alt"></i> Compare
//                       </button>
...
//                       <span>Share: </span>
//                       <a href="#!" onClick={handleShareOnWhatsApp}>
//                         <i className="lab la-whatsapp" style={{ fontSize: '25px', color: 'green' }}></i>
//                       </a>
//                       <a href="#!" onClick={handleShareOnYouTube}>
...
//                         <button className="nav-link" id="nav-description-tab" data-bs-toggle="tab" data-bs-target="#nav-description" type="button" role="tab" aria-controls="nav-description" aria-selected="true">Download Brochure</button>
//                         <button className="nav-link active" id="nav-addInfo-tab" data-bs-toggle="tab" data-bs-target="#nav-addInfo" type="button" role="tab" aria-controls="nav-addInfo" aria-selected="false">Specifications</button>
...
//                               </div>
//                               {brochureUrl ? (
//                                 <a target="_blank" href={brochureUrl} className="download-button-svg" rel="noreferrer" download>
//                                   <svg className="download-svg" viewBox="0 0 24 24">
...
//                                   </div>
//                                   <div style={{ fontSize: 18 }}>{renderStars(Math.round(ratingStats.average))}</div>
...
//                                       <input type="number" name="rating" min="1" max="5" placeholder="Enter your rating here!" required />
//                                     </div>
//                                   </div>
...
//             <button className="login-dialog-close" onClick={handleDialogClose}>
//               &times;
//             </button>
//             <h3>Please Log In</h3>
//             <p>
//               You need to log in to {dialogAction === 'cart' ? 'add this product to your cart.' : 'add this product to your wishlist.'}
//             </p>
//             <div className="dialog-actions">
//               <button onClick={handleDialogClose}>Cancel</button>
//               <button onClick={handleDialogLogin}>Log In</button>
//             </div>
//           </div>
//         </div>
//       )}
